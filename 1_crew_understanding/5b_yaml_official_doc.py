...
## BELOW CODE IS AS PER THE CREWAI DOCUMENTATION
...
class BlogCrew():
    ...
    def researcher(self) -> Agent:
        print(f"self.agents_config: {self.agents_config}")  # self.agents_config: {'research_agent': {'llm': 'gemini/gemini-2.0-flash', 'role': 'Research Specialist\n', 'goal': 'Research interesting facts about the topic: {topic}\n', 'backstory': 'You are an expert at finding relevant and factual data\n'}, 'writer_agent': {'llm': 'gemini/gemini-2.0-flash', 'role': 'Creative Writer\n', 'goal': 'Write a short blog summary using the research\n', 'backstory': 'You are skilled at writing engaging summaries based on provided content.'}}
        return Agent(
...
if __name__ == "__main__":
    ...
    blog_crew.crew().kickoff(inputs=inputs)
...
# 3. Automation & Defaults
    # No explicit paths to the YAML files are set because:

        # The @CrewBase decorator defaults to config/agents.yaml and config/tasks.yaml.
        # Method names (e.g. researcher, research_task) must match the keys in the YAML files for implicit wiring.
